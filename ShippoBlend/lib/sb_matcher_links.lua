--[[
  sb_matcher_links.lua
  Shippo Blend Plugins — sb_matcher の一部。リンク・エイリアスの合成（LWW）、検索、
  近い名前（"≒"印）の判定。sb_matcher.lua が require して自分のAPIに合成する
  （モジュールを350行未満に保つための分割。呼び出し側は sb_matcher だけ見ればよい）。
--]]

local normalize = require("sb_normalize")
...
-- の形（sb_store は中身を関知しないので、この形はSendNow/UI側で書く約束にする）。
...
-- intersection_fn は sb_matcher.intersection（循環requireを避けるため呼び出し側から渡す）。
