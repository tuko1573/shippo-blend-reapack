--[[
  sb_matcher.lua
  Shippo Blend Plugins — 全員分の member ドキュメントから「誰が何を持っているか」の索引を作り、
...
local links_part = require("sb_matcher_links")

local M = {}

-- そのまま公開する（sb_matcher_links.M.search は circular require を避けるため
...
-- sb_normalize.loose_key と同じ規則だが、既にkeyになっている文字列に対して直接適用する
...
--- sb_matcher_links.search はcircular requireを避けるため intersection を引数で受け取る。
