--[[
  sb_langpack.lua
  Shippo Blend Plugins — 日本語の言語ファイル（ReaperLangPack）を自動で入れ替える／切り替える。

  やること（REAPER起動のたび、SendNowの最後から呼ばれる）:
    1. reaper.ini の langpack= を読む（ネットは使わない）
       - Shippo Blend 標準の日本語パッチを指している → "update"（1日1回だけ配布元を確認）
...
function M.decide(mode, kind, code, dest_exists, verbose, quiet_fail)
  ...
  if mode == "switch" then
    if dest_exists then
      r.set_ini = true
      r.msg = "Shippo Blend 標準の日本語パッチに切り替えました。\n\n"
...
function M.run(deps, opts)
  ...
  local check_file = work .. "/last_check.txt"
  ...
  local last = deps.read_file(check_file)
  ...
  deps.mkdir_p(work)
  deps.write_file(check_file, deps.today())
...
--- 実機のREAPERから deps を組み立てる（sb_bootstrap に寄せるとImGui側にも依存が増えるのでここに置く）。
