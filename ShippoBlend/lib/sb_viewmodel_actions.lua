...
-- 生の名前の接頭辞 → 形式トークン（sb_normalize の PREFIXES と同じ対応）。
local PREFIX_TO_FMT = {
...
function M.toggle_unusable(state, key, store, root, now_iso)
  ...
  if not doc then return false, "自分の一覧がまだDropboxにありません（設定タブの「今すぐ更新」を先に）" end
...
-- 全員分を合わせたとき（sb_matcher.merge_aliases）にこちらが勝ち、エイリアスが消える。
-- @return ok, err
