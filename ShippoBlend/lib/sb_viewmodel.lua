--[[
  sb_viewmodel.lua
  Shippo Blend Plugins — 小窓（sb_ui）が表示するものを、データだけから組み立てる純Lua部分。
  REAPER も ImGui も呼ばない（テストが素のLuaで走る）。突き合わせの規則そのものは
  sb_matcher / sb_store / sb_normalize が持ち、ここでは「画面に出す形」に整えるだけ
  ＝ロジックをUI側へ複製しない。

  参照: 計画書/計画書.md 2章（ゴール）・3-2/3-4節。
--]]
...
-- 「押したときに起きること」は sb_viewmodel_actions に置いてある（同じAPIとして公開する）。
...
-- sb_matcher.merge はエイリアス解決と loose_key の畳み込みでキーを書き換えることがあるので、
...
--- @param store  sb_store のインスタンス
-- @param root   store:root(dropbox_path) の結果
-- @param config sb_config のインスタンス（member_id / display_name を読む）
